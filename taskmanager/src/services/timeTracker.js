...
class TimeTrackerService {
    ...
    try {
      ...
      for (const task of tasks) {
        if (task.isTimerRunning && task.timerStartTime) {
          const startTime = moment(task.timerStartTime);
          ...
          await this.updateTaskTimeSpent(task.id, elapsedTime);
        }
      }
      ...
      console.log('Time tracker initialized with', this.activeTimers.size, 'active timers');
    } catch (error) {
  ...
  async scheduleTimeUpNotification(taskId, estimatedDuration) {
    try {
      ...
      await notificationService.scheduleNotification({
        ...
        content: {
          title: 'Time Up!',
          body: `Estimated time for "${task.title}" has elapsed`,
  ...
  async handleTimeUp(taskId, task) {
    try {
      ...
      await notificationService.scheduleNotification({
        identifier: `time_up_immediate_${taskId}`,
        content: {
          title: 'Time Up!',
          body: `Estimated time for "${task.title}" has elapsed`,
          data: { taskId, type: 'time_up' }
